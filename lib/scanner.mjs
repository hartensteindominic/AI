...
function dispositionOf({ title, text, labels, source, assigneeCount }) {
  const header = `${title}\n${labels}\n${source}`;
  const bountySignal = /\b(?:bounty|reward)\b/i.test(header);
  const grantRecord = /\bgrants?\b|grant application|grant proposal|funding request/i.test(header)
    || /applicant information|requested grant amount|grant overview|grant team|grant mission/i.test(text);

  if (grantRecord && !bountySignal) return 'Grant ownership record';

  const awardedHeader = /\b(?:awarded|approved|paid|work in progress|in progress)\b/i.test(header);
  const awardedBody = /milestone\s+\d+[^\n]*(?:closed\s*\/\s*paid|in progress)|being processed as a partial release/i.test(text);
  if (awardedHeader || awardedBody) return 'Already awarded or in progress';

  if (assigneeCount > 0) return 'Already assigned';

  const taskSignal = /\b(?:bounty|reward|paid task|contract(?:or)?|help wanted|good first issue|fix|implement|add|build|create|develop|refactor|debug|test|document|upgrade|migrate|improve|optimi[sz]e|repair|resolve)\b/i.test(`${title}\n${labels}`);
  if (!taskSignal) return 'No open-task signal';
...
function risksOf(text, ageDays, comments) {
  const risks = [];
...
function hoursOf(text) {
  const explicit = text.match(/(?:estimate|estimated|effort|time|covers?)\D{0,16}(\d+(?:\.\d+)?)\s*(?:h|hours?)/i);
...
function competitionOf(comments) {
...
export async function scanOpportunities(options = {}) {
  ...
  const now = Date.now();
  const analyzed = Array.from(unique.values()).map((issue) => {
    const labels = (issue.labels || []).map((label) => typeof label === 'string' ? label : label.name || '').join(' ');
    const text = `${issue.title}\n${issue.body || ''}\n${labels}`;
    const source = repoName(issue.repository_url);
    ...
    const riskFlags = risksOf(text, ageDaysExact, comments);
    ...
    const competition = comments >= 15 ? 0.68 : comments >= 6 ? 0.82 : 1;
    ...
    return {
      ...
      hours,
      probability,
      competition: competitionOf(comments),
  ...
  const rejectionSummary = analyzed.reduce((summary, opportunity) => {
    if (opportunity.disposition) {
      summary[opportunity.disposition] = (summary[opportunity.disposition] || 0) + 1;
    }
    return summary;
  }, {});

  const opportunities = analyzed.filter((opportunity) => (
    ...
    || b.payout - a.payout
  )).slice(0, maxResults).map(({ disposition, ...opportunity }) => opportunity);
  ...
  return {
    ...
    rejectionSummary,
    methodology: 'Open + explicit payout + linked/stated protection + updated within 45 days. Excludes grant ownership records, awarded/in-progress work, assigned issues, and listings without an open-task signal. Ranking adjusts expected return for clarity, freshness, visible competition, and automation fit.',
  };
}
